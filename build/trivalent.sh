...
# prefer distro-appropriate libdir for supplemental assets
detect_trivalent_libdir() {
  local candidates=(
...
declare -r TRIVALENT_LIBDIR="$(detect_trivalent_libdir)"
...
if [[ -f "$TRIVALENT_LIBDIR/trivalent/install_filter.sh" ]] ; then
  /bin/bash "$TRIVALENT_LIBDIR/trivalent/install_filter.sh"
fi
